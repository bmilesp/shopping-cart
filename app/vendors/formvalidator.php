...
**/
class CustomValidator extends Connections
{
...
class PasswordConfirmValidator extends CustomValidator
{
    function DoValidate(&$formars,&$error_hash)
    {
        if($formars['password'] != $formars['password_confirm'])
        {
            $error_hash['password_confirm']="Passwords do not match";
            return false;
        }
    return true;
    }
...
{
	var $postvar_name = 'email';
	var $field_name = 'email';
	var $table_name = 'table';
	var $db_name = 'admin';
	
	function UniqueValidator($field_name = 'email', $table_name = 'users',$postvar_name = 'email',$db_name ='admin'){
		parent::__construct();
		$this->postvar_name = $postvar_name;
    ...
    {
    	$return = true;
    	echo $q ="SELECT {$this->field_name}, retail_password from {$this->db_name}.{$this->table_name} where email = '{$_POST[$this->postvar_name]}' limit 1";
    	$r = mysql_query($q);
		$row = mysql_fetch_assoc($r);
		if(isset($row['email']) && !empty($row['retail_password'])){
			$error_hash[$this->postvar_name] = 'Email address already exists in our system. Please use another email address';
			 $return = false;
		}
    	return $return;
    }
...
class PasswordVarifyValidator extends CustomValidator
{
	var $postvar_name = null;
	var $field_name = null;
	var $table_name = null;
	var $errorMessage = null;
    
	function PasswordVarifyValidator($field_name = 'password', 
                                  $table_name = 'customers',$postvar_name = 'old_password',
							 $errorMessage = 'Your old password does not match the password in our system. Please try again.'
							 ){
		parent::__construct();
		$this->postvar_name = $postvar_name;
		$this->table_name = $table_name;
		$this->field_name = $field_name;
		$this->user_id = isset($_POST['cust_id'])? $_POST['cust_id'] : null;
		$this->errorMessage = $errorMessage;
	}
	
    function DoValidate(&$formars,&$error_hash)
    {
    	$cust_id = ($this->user_id)? "AND `cust_id` = {$this->user_id}" : null;
    	if($this->security_salt){
    		$_POST[$this->postvar_name] = hash('md5',stripslashes($_POST[$this->postvar_name]).$this->security_salt);
    	}
    	
    	$q ="SELECT {$this->field_name} 
    		from {$this->table_name} 
    		where {$this->field_name} = '{$_POST[$this->postvar_name]}'
    		$cust_id limit 1";
    	$r = mysql_query($q);
		$row = mysql_fetch_assoc($r);
		if(!isset($row[$this->field_name])){
			$error_hash[$this->postvar_name] = $this->errorMessage;
			 return false;
		}
    return true;
    }
...
class FormValidator 
{
	...
	function outputError($field,$class='errorEmptyRequiredField'){
		if(isset($this->error_hash[$field])){
			return "<span class='$class'>{$this->error_hash[$field]}<span>";
		}
	}
	
	function msgConfirmPasswords($pass1, $pass2, $message = "Passwords do not match", $class = 'errorEmptyRequiredField'){
		if($pass1 != $pass2){
